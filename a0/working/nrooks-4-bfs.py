...
def successors3(board):
    ...
    if total_pieces < N:
        return [add_piece(board, r, total_pieces) for r in range(0, N) if board[r][total_pieces]!=1]
    else:
        return[]
...
def solve(initial_board):
    ...
    while len(fringe) > 0:
        for s in successors3( fringe.pop(0) ):  # Note: .pop() is for DFS, .pop(0) is for BFS
            if is_goal(s):
                return(s)
            fringe.append(s)
    return False
